...
library(base64enc) # for working with html images
library(gt)
library(gtExtras)
library(gtUtils)
...
<div style="position: relative; display: inline-block; width: 300px;">
  ...
  <div style="position: absolute; bottom: -42px; right: 5px;
    ...
    <img src="`r team_logo_url`" width="65"
         style="display: block; margin: auto; padding-top: 7px;"/>
  </div>
</div>
```
:::
...
player_stats_total |> 
  ...
  mutate(
    ...
    OREB_pg = or_pg |> round(1),
    DREB_pg = dr_pg |> round(1),
...
player_stats_total |> 
  ...
  select(
    fgm, fga, fg_per, fgm2, fga2, fg2_per, fgm3, fga3, fg3_per, ft_made, ft_att, ft_per, efg_per
  ) |> # turn into a nice gt table
  gt() |> 
  # format column names
  cols_label(
...
player_stats_total |> 
  ...
  mutate(
    short2_FG_per = (rim_fg_per/100) |> round(digits = 3),
    long2_FG_per = (non_rim2_fg_per/100) |> round(digits = 3)
  ) |> 
  select(
...
player_stats_vs_tournament |> 
  ...
  select(
    FGM, FGA, FG_per, FGM3, FGA3, FG3_per, FTM, FTA, FT_per, EFG_per
  ) |> # turn into a nice gt table
  gt() |> 
  # format column names
  cols_label(
